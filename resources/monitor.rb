property :monitor_name, String, name_property: true
...
action :create do
  ...
  if monitor.monitor_is_missing?(new_resource.monitor_name)
    converge_by "Create monitor template #{new_resource.monitor_name}" do
      monitor.create_monitor(new_resource.monitor_name, **f5_attributes)
    end
  end
  ...
  if monitor.common_attributes_changed?(new_resource.monitor_name, **changeable_attributes)
    converge_by "Update common attributes on monitor template #{new_resource.monitor_name}" do
      monitor.update_common_attributes(new_resource.monitor_name, **changeable_attributes)
    end
  end

  mismatches = monitor.string_properties_match?(new_resource.monitor_name, new_resource.string_properties)
  unless mismatches.empty?
    msg = "The following string properties are being updated: #{mismatches.join ', '}"
    converge_by msg do
      monitor.update_string_properties(new_resource.monitor_name, mismatches, new_resource.string_properties)
    end
  end

  if monitor.timeout_changed?(new_resource.monitor_name, new_resource.timeout)
    converge_by 'Updating timeout' do
      monitor.update_timeout(new_resource.monitor_name, new_resource.timeout)
    end
  end

  if monitor.interval_changed?(new_resource.monitor_name, new_resource.interval)
    converge_by 'Updating interval' do
      monitor.update_interval(new_resource.monitor_name, new_resource.interval)
    end
  end
end
...
action :destroy do
  load_f5_gem
  monitor = ChefF5::Monitor.new(node, new_resource, new_resource.load_balancer, new_resource.partition)
  unless monitor.monitor_is_missing?(new_resource.monitor_name)
    converge_by "Deleting monitor template #{new_resource.monitor_name}" do
      monitor.delete_monitor(new_resource.monitor_name)
    end
  end
end
